...
void update_led()
{
  if (dtmf_cmd == '1') {
    if (LED1_STATE) {
      LED1_STATE = false;
      digitalWrite(LED1, LED1_STATE); //relay 1 on
  ...
  if (dtmf_cmd == '2') {
    if (LED2_STATE) {
      LED2_STATE = false;
      digitalWrite(LED2, LED2_STATE); //relay 2 on
...
if (dtmf_cmd == '3') {
    if (LED3_STATE) {
      LED3_STATE = false;
      digitalWrite(LED3, LED3_STATE); //relay 1 on
      Serial.println("RELAY 3 OFF");
    } else {
      LED3_STATE = true;
      digitalWrite(LED3, LED3_STATE); //relay 3 on
  ...
  if (dtmf_cmd == '4') {
    if (LED4_STATE) {
      LED4_STATE = false;
      digitalWrite(LED4, LED4_STATE); //relay 4 on
